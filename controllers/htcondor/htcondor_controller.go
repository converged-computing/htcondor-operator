...
// HTCondorReconciler reconciles a HTCondor object
type HTCondorReconciler struct {
	client.Client
...
//+kubebuilder:rbac:groups=flux-framework.org,resources=htcondors,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=flux-framework.org,resources=htcondors/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=flux-framework.org,resources=htcondors/finalizers,verbs=update
...
// the HTCondor object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.14.1/pkg/reconcile
func (r *HTCondorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	// Create a new HTCondor
	var cluster api.HTCondor

	// Keep developer informed what is going on.
	r.Log.Info("🦕 Event received by HTCondor controller!")
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			r.Log.Info("👑️ HTCondor not found . Ignoring since object must be deleted.")
			return ctrl.Result{}, nil
		}
		r.Log.Info("👑️ Failed to get HTCondor. Re-running reconcile.")
		return ctrl.Result{Requeue: true}, err
	}

	// Don't continue if they provided 0 size, that makes no sense!
	if cluster.Spec.Size == 0 {
		r.Log.Info("👑️ A HTCondor must have at least one node")
		return ctrl.Result{}, nil
	}

	// Show parameters provided and validate one flux runner
	if !cluster.Validate() {
		r.Log.Info("👑️ Your HTCondor config did not validate.")
		return ctrl.Result{}, nil
	}

	// Ensure we have the HTCondor cluster
	result, err := r.ensureHTCondor(ctx, &cluster)
	if err != nil {
		return result, err
	}
	...
	r.Log.Info("👑️ HTCondor is Ready!")

	return ctrl.Result{}, nil
}

// SetupWithManager sets up the controller with the Manager.
func (r *HTCondorReconciler) SetupWithManager(mgr ctrl.Manager) error {
